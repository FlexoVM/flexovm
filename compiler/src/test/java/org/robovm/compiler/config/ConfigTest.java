...
public class ConfigTest {
    ...
    private File createMergeConfig(File tmpDir, String dir, String id, OS os, Arch arch, boolean jar) throws Exception {
        ...
        for (OS os2 : OS.values()) {
            for (Arch arch2 : Arch.values()) {
                File root = new File(p, "META-INF/flexovm/" + os2 + "/" + arch2);
                root.mkdirs();
                if (!new File(root, "flexovm.xml").exists()) {
                    new Config.Builder().write(new File(root, "flexovm.xml"));
                }
            }
        }

        File root = new File(p, "META-INF/flexovm/" + os + "/" + arch);
        new Config.Builder()
            ...
            .write(new File(root, "flexovm.xml"));
    ...
    public void testMergeConfigsFromClasspath() throws Exception {
        ...
        File p1X86Root = new File(p1, "META-INF/flexovm/macosx/x86");
        File p3X86Cache = config.getCacheDir(config.getClazzes().getClasspathPaths().get(2));
        File p3X86Root = new File(p3X86Cache.getParentFile(), p3X86Cache.getName() + ".extracted/META-INF/flexovm/macosx/x86");
        ...
        File p3X86_64Root = new File(p3X86_64Cache.getParentFile(), p3X86_64Cache.getName() + ".extracted/META-INF/flexovm/macosx/x86_64");
